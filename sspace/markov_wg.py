...
def similarity_score (id1, ids):
    ...
    for id in ids:
        if (' ' .join(mydata.titles_test[id1]),' '.join(mydata.titles_train[id])) not in simdic:
            sim =  ngram_simscore(mydata.titles_test[id1], mydata.titles_train[id])
...
def predict(lis, id):
    ...
    if history in models[order-1].keys():
        ...
        prediction = max(models[order-1][history].keys(), key=(lambda k: landa * np.log(models[order-1][history][k][0]/normp) + (1-landa)*np.log(similarity_score(id,models[order-1][history][k][1])/norms)))
        return prediction
    elif len(lis) > 0:
        lis2 = lis[1:]
        predict(lis2, id)
...
if __name__ == '__main__':
    write_result('/home/nnabizad/code/toolpred/sspace/res/mac/akom_wig_landa.txt')
    save_obj(simdic, 'ngram-similarities')
