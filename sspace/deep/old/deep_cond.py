...
class VariableSequenceLabelling:
    ...
    def prediction(self):
        ...
        # tf.eval(tf.shape(output))

        # Softmax layer.
        max_length = int(self.target.get_shape()[1])
        num_classes = int(self.target.get_shape()[2])
        weight, bias = self._weight_and_bias((self._num_hidden+self._num_thidden), num_classes)

        titles_output = tf.reshape(tf.tile(titles_output[:, -1, :], [1, max_length]), [-1, max_length, self._num_thidden])
        # Flatten to apply same weights to all time steps.
        output = tf.concat([output, titles_output], axis=-1)
        output = tf.reshape(output, [-1, self._num_hidden+self._num_thidden])
    ...
    for seed in seeds:
        ...
        for epoch in range(200):
            ...
            if new_accu > old_accu and epoch > 5:
                # saver.save(sess, "models/models_lstm/conditioned/withoutnotool/Maclaptob_{0}.ckpt".format(seed), write_meta_graph=False)
                old_accu = new_accu
                best_epoch = epoch
