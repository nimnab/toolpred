...
def q1(freq_1gram, freq_2gram):
    q1_wc = {'colorless':0, 'green':0, 'ideas':0, 'sleep':0, 'furiously':0, END_TOK:0}
    q1_biwc = {('colorless', 'green'):0, ('green','ideas'):0, ('ideas','sleep'):0, ('sleep','furiously'):0, ('furiously',END_TOK):0}
    uni_mle = 0
    for key in q1_wc:
...
def get_stats(freq_1gram, tmat, vocab, num_classes):
    ...
    for c in range(num_classes):
        ...
        for x in range(len(srt_wrds_c)):
            ...
            if w in top_wrds and w != END_TOK and w != START_TOK:
                topk[w] = pr
...
def check_params(freq_1gram, freq_2gram, cfreq_2gram, cfreq_2gram_w2):
    ...
    assert cfreq_2gram[END_TOK].N() == 0, 'there shld be no bigrams w1=END_TOK'
    assert len(cfreq_2gram_w2.get(START_TOK, {})) == 0, 'there should be no bigrams w2=START_TOK'
...
def train(biglis, num_classes, emitr):
    num_sents, pos = 0, 0
    ...
    for sent in biglis:
        ...
        for i, wrd in enumerate(sent):
            ...
            if wrd == END_TOK and i !=len(sent)-1: print("@@@", sent)
            sent_tok.append(wrd)
            if wrd not in vocab:
                vocab[wrd] = pos
                pos += 1
...
if __name__ == '__main__':
    ...
    train(mydata.train, LATENT_CLASSES, EM_ITERATIONS)
    # test()